...
export function postOrder(req: Req, res: Response) {
  const order: Order = req.body
  const userId = req.user.sub
  redisdb.createItem(type, JSON.stringify(order)).then((uid) => {
    redisdb.addSetItem(`order:user:${userId}`, `order:${uid}`).then((_orderUserLink) => {
...
export function getMyOrders(_req: Req, res: Response) {
  const userId = _req.user.sub
  redisdb.getSmembers(`order:user:${userId}`).then((orders) => {
...
export function getAllOrders(_req: Req, res: Response) {
  redisdb.getKeys(`order:user:*`).then((keys) => {
