...
// decorate class to unsubscribe looking at all properties
export function AutoUnsubscribeV2(_destroyFunc = 'ngOnDestroy', _subscriptionsArray = '_subscriptions') {
  return function (constructor) {
    const original = constructor.prototype[_destroyFunc]
    // constructor.prototype[_subscriptionsArray] = ['xxx', 'yyy']
    if (!original || typeof original !== 'function') {
      console.log(console.error('You must add function <', _destroyFunc, '> method to use AutoUnsubscribe'))
      return original.apply(this, arguments)
    } else {
      constructor.prototype[_destroyFunc] = function () {
        ...
        if (subArray) {
          ...
          console.error('You must add property <', _subscriptionsArray, '> method to use AutoUnsubscribe')
        }
        return original.apply(this, arguments)
      }
    }
...
export function AutoUnsubscribeFactory(verbose = true) {
  ...
  return function (constructor) {
    ...
    const original = constructor.prototype.ngOnDestroy
    if (!original || typeof original !== 'function') {
      if (verbose) { console.log('You must implement ngOnDestroy method to use AutoUnsubscribe') }
    } else {
      constructor.prototype.ngOnDestroy = function () {
        Object.keys(this).forEach((prop) => {
          const property = this[prop]
